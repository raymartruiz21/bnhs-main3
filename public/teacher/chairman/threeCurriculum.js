            ...
            },
            {
                ...
                render: function (data) {
                    switch (data.enroll_status) {
                        case "Pending":
                            return `<span class="badge bg-warning pb-2">${data.enroll_status}</span>`;
                            break;
                        case "Enrolled":
                            return `<span class="badge bg-success pb-2">${data.enroll_status}</span>`;
                            break;
                        case "Dropped":
                            return `<span class="badge bg-danger" pb-2>${data.enroll_status}</span>`;
                            break;
                        default:
                            return false;
                            break;
                ...
                render: function (data) {
                   if (data.state=="Old") {
                    ...
                    if (data.req_grade != null || data.req_goodmoral != null || data.req_psa != null) {
                        ...
                        return `
                            <button type="button" class="btn btn-warning text-white btn-sm pt-0 pb-0 pl-3 pr-3 btnRequirement" value="${data.fullname + "^" + data.req_grade + '^' + data.req_goodmoral + '^' + data.req_psa}"><i class="fas fa-file-import"></i> view</button>
                          `;
                        } else {
                        return '--- None ---';
                    }
            ...
            },
            {
                ...
                render: function (data) {
                    if (data.enroll_status == "Dropped") {
                        return `<button type="button" class="btn btn-sm btn-danger text-white cDelete btnDelete_${data.id}  pt-0 pb-0 pl-2 pr-2" id="${data.id}">
                        Delete
                        </button>
                        `;
                    } else {
                       ...
                       ${
                           data.enroll_status == "Enrolled"
                               ? ` <button type="button" class="btn btn-sm btn-info text-white cEdit btnEdit_${data.id} pt-0 pb-0 pl-3 pr-3 " id="${data.id}"> Change </button>`
